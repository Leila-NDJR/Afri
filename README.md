...
**Note de vérification** : la syntaxe de `dashboard.py` a été validée (`python -m py_compile dashboard.py`, sans erreur) et sa logique relit fidèlement `scoring_utils.py`/`categorisation.py` sur les vraies données. Son exécution réelle a également été testée avec succès (`streamlit run dashboard.py`, lancé en headless, réponse HTTP 200 ; les 4 pages ont aussi été rejouées sans exception via `streamlit.testing.v1.AppTest`) — habillage visuel (thème pastel, palette `PASTEL`) inclus. La suite de tests (`pytest tests/`) passe intégralement (73 tests).
